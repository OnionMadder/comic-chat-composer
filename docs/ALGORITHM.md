...
Backdrop selection (`chooseBackdrop` in [`src/compose.ts`](../src/compose.ts)) happens at each establishing shot, and the conversation that follows inherits it — so the establishing shot establishes the place the scene actually happens in, rather than the scene cutting to a different backdrop. The present cast's `backdropPreferences` steer the choice: a backdrop ranked `r` (most-preferred first) in a character's list scores `listLength - r`, summed across the panel's characters, and the highest-scoring backdrop that isn't the previous one wins (ties broken by the seeded RNG). With no manifests it degrades to non-repeating rotation, which is the original behaviour.
