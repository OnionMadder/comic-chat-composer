...
**Extended.** `EXTRA_POSE_RULES` adds emoji, more emoticons, later acronyms (`ROFL`/`LMAO`/`IMHO`/`BRB`), more question openers, casual greetings, and **written-out laughter** — at the shipped table's strength bands. That last one is a gap rather than an anachronism: the shipped table has `ROTFL`, `LOL` and `HEHE` but nothing for `haha`, the commonest written laugh there is, so `"Ha ha ha, scaredy cat!"` came out perfectly neutral. A `find` on `HAHA` (strength 11, with the laughs) covers every length and prefix — `hahaha`, `ahaha`, `bwahahaha` — and a `word` rule on `ha ha` covers the spaced form, which no substring search can see. The doubled stem is the whole test; a lone `ha` is as often dismissive as amused and stays neutral. It also adds rules for `angry`, `scared` and `bored`, which **no released version could trigger from text**: those three rule strings are empty in v1.0, v2.1b and v2.5 alike, so only the emotion wheel could ever set them. (A side effect, pinned in `test/pose.test.ts`: under the shipped table alone, `>:(` renders *sad*, because its `:(` tail matches the frown rule and nothing outranks it.)
...
- **A B-spline does not pass through its control points.** Feeding the balloon boundary in directly yields an outline visibly *tighter* than intended — the widest line of text spills outside the balloon drawn to hold it. `fitControlPoints` corrects for this by iterative refinement: evaluate where the curve actually lands, push each control point by the shortfall, repeat. The correction is capped and bounded so a sharp contour change can't throw a control point out into a spike. That bound has to describe the *solution*, not the input: interpolating a knot always means placing its control point beyond the knot, so a knot at the boundary's own extremum — a cap shoulder, in practice — needs a control point outside the target polygon entirely. Bounding by the targets' extent plus a fixed slack instead made the clamp bind on exactly those points, and the outline quietly fell short of the boundary it was fitted to.
